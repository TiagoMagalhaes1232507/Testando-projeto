...
describe("Posts Endpoints", () => {
  ...
  describe("POST / Upvote Post", () => {
    test("CTP26 - should increment points after upvote", async () => {
    ...
    test("CTP27 - should update the voteByMe field after upvote", async () => {
      // ACT: Trigger the upvote endpoint
      const uri = "/api/v1/posts/upvote";
      await request(url)
        .post(uri)
    ...
    test("CTP28 - should return status 403 for expired token", async () => {
    ...
    test("CTP29 - should return status 404 for non-existent post", async () => {
    ...
    test("CTP30 - should return status 400 if userId is included in the request body", async () => {
